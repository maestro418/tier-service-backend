...
export const getNeedDate = () => {
	const now = Now();
	const lastYearStart = new Date(now * 1000);
	lastYearStart.setFullYear(lastYearStart.getFullYear() - 1, 0, 1);
	const lastYearEnd = new Date(now * 1000);
	lastYearEnd.setFullYear(lastYearEnd.getFullYear() - 1, 11, 31);
	const thisYearStart = new Date(now * 1000);
	thisYearStart.setFullYear(lastYearStart.getFullYear(), 0, 1);
	const thisYearEnd = new Date(now * 1000);
	thisYearEnd.setFullYear(thisYearEnd.getFullYear(), 11, 31);
	return { lastStart: lastYearStart.getTime() / 1000, lastEnd: lastYearEnd.getTime() / 1000, thisStart: thisYearStart.getTime() / 1000, thisEnd: thisYearEnd.getTime() / 1000 };
};
